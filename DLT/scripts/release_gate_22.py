...
def main()->int:
    ...
    a=p.parse_args()
    src=json.loads(Path(a.gate_input).read_text(encoding="utf-8-sig")).get("gates",{})
    gates={k:src.get(k,"UNKNOWN") for k in HARD_GATES}
    acceptance=json.loads(Path(a.acceptance).read_text(encoding="utf-8-sig"))
    exe=Path(a.exe)
    actual=sha256(exe) if exe.exists() else None
    if acceptance.get("final_release_gate")!="PASS" or int(acceptance.get("hard_fail_count",999))!=0:
        gates["exact_exe"]="FAIL"
    if not actual or actual!=acceptance.get("exe_sha256"):
        gates["same_hash"]="FAIL"
    failures={k:v for k,v in gates.items() if v!="PASS"}
    report={"final_gate":"PASS" if not failures else "FAIL","hard_fail_count":len(failures),"gates":gates,"exe_sha256":actual,"failures":failures}
